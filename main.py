...
class klasa_cokolwiek(tk.Frame):
    def __init__(self, parent, controller, address=1):
        ...
        self.ready = False
        self.LENGTH = 200  # Długość nici wahadła
        self.RADIUS = 10  # Promień kuli wahadła
    ...
    def dodaj_pkt(self):
        ...
        for x in range(1000000):
            y=x/10
            self.axs[0].set_xlim( y- 3, y + 1)
            self.canvas.draw()
            self.slupekBox.coords(self.kinetyczna, 75, 269 - (self.wartosci[x] * 100), 230, 270)
            self.slupekBox.coords(self.potencjalna, 270, 269 - ((2-self.wartosci[x]) * 100), 415, 270)
    ...
    def plot_Energia(self):
        ...
        self.scatter1 = self.axs[1].plot(self.czas3, 2-self.wartosci)  # kinetyczna liniowa
